...
class BatchNorm1dOp : public Op {
  ...
  mlengine::MatrixRM *running_mean_, *running_var_;

  mlengine::MatrixRM x_centered_, stddev_inv_, x_hat_;

 public:
  BatchNorm1dOp(Tensor* x, Tensor* gamma, Tensor* beta, Tensor* out,
                mlengine::MatrixRM* running_mean,
                mlengine::MatrixRM* running_var, float momentum, float eps,
    ...
    if (*is_training_) {
      ...
      *running_var_ = (1.0f - momentum_) * running_var_->array() +
      ...
      mlengine::MatrixRM x_hat_eval =
          ...
          std_inv.row(0).array();
      out_->data = (x_hat_eval.array().rowwise() * gamma_->data.row(0).array())
                       .rowwise() +
                   beta_->data.row(0).array();
    }
  }
  ...
  void backward() override {
    ...
    if (x_->requires_grad) {
      ...
      term.array() -=
          x_hat_.array().rowwise() * dx_hat_x_hat_sum.row(0).array();

      // FIXED: Apply the scalar (1.0f / batch) to the 1D vector before rowwise
      // broadcasting
      x_->grad.array() += term.array().rowwise() *
                          (stddev_inv_.row(0).array() * (1.0f / batch));
    }
  }
};
