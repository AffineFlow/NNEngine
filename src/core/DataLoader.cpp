...
void DataLoader::next_batch(autograd::Tensor& X_batch,
  ...
  affineflow::nn::Shape expected_x = X_.shape;
  expected_x[0] = actual_batch_size;
  if (X_batch.shape != expected_x) {
    X_batch.resize(expected_x);
  }

  affineflow::nn::Shape expected_y = y_.shape;
  expected_y[0] = actual_batch_size;
  if (y_batch.shape != expected_y) {
    y_batch.resize(expected_y);
  }

  Eigen::Index x_row_size = affineflow::nn::compute_size(X_.shape) / X_.shape[0];
  Eigen::Index y_row_size = affineflow::nn::compute_size(y_.shape) / y_.shape[0];
