...
Conv2dLayer::Conv2dLayer(int in_channels, int out_channels, int in_h, int in_w,
    ...
    : w_(affineflow::nn::Shape{out_channels, in_channels * kernel_size * kernel_size},
         true),
      bias_(affineflow::nn::Shape{1, out_channels}, true),
...
autograd::Tensor* Conv2dLayer::forward(autograd::Tensor* input) {
  ...
  auto* out = tape->alloc_tensor(
      affineflow::nn::Shape{input->shape[0], out_channels_ * out_h * out_w},
      req_grad);

  auto* op = tape->allocate_op<affineflow::nn::autograd::ops::Conv2dOp>(
