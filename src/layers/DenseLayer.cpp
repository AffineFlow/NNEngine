...
DenseLayer::DenseLayer(int input_dim, int output_dim)
    : weights_(affineflow::nn::Shape{input_dim, output_dim}, true, true),
      bias_(affineflow::nn::Shape{output_dim}, true, false) {
...
autograd::Tensor* DenseLayer::forward(autograd::Tensor* input) {
  ...
  auto* mm = tape->alloc_tensor(
      affineflow::nn::Shape{input->shape[0], weights_.shape[1]}, req_grad_mm);
  auto* mm_op = tape->allocate_op<affineflow::nn::autograd::ops::MatMulOp>(
      input, &weights_, mm);
  mm_op->forward();
  ...
  auto* bias_op =
      tape->allocate_op<affineflow::nn::autograd::ops::AddBiasOp>(mm, &bias_, out);
  bias_op->forward();
