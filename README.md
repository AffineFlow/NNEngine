...
[![PyPI version](https://img.shields.io/pypi/v/AffineFlow-NN?logo=pypi&logoColor=white)](https://pypi.org/project/AffineFlow-NN/)
[![Python](https://img.shields.io/pypi/pyversions/AffineFlow-NN?logo=python&logoColor=white)](https://pypi.org/project/AffineFlow-NN/)
...
Designed for rapid experimentation without the Python Global Interpreter Lock (GIL) overhead, `AffineFlow-NN` executes the entire deep learning training loop strictly in native C++ using Eigen. It utilizes a zero-allocation flat-memory Autograd graph, AVX SIMD vectorization, and dynamically compiled OpenBLAS to achieve massive speedups over mainstream Python frameworks.

## Highlights

- **Dual Execution Modes**: Train dynamic graphs using the `@nn.eager` decorator for zero-allocation step-by-step execution, or compile static graphs with `JITCompiler` for maximum throughput.
...
| Benchmark Task | Scikit-Learn | PyTorch (ATen) | AffineFlow-NN (Eager) | AffineFlow-NN (C++ JIT) | Max Speedup (vs PyTorch) |
|---|--:|--:|--:|--:|--:|
